...
def train_or_eval_model(model, embeddings, dataloader, epoch, loss_function=None, optimizer=None, train=False,
...
if __name__ == '__main__':
    path = '../experiments/ResidualGCN_baseLSTM_EC/'
    ...
    if args.tensorboard:
        ...
        writer = SummaryWriter('../tb_logger/ResidualGCN_baseLSTM_EC')
    ...
    logger.info('D_emb {} | D_m {} | D_r {} | D_g {} | D_p {} | D_e {} | D_h {} | graph_h {} | D_a {}'.format(D_emb, D_m, D_r, D_g, D_p, D_e, D_h, graph_h, D_a))
    ...
    all_fscore, all_fscore_micro, all_acc, all_loss = [], [], [], []

    for e in range(n_epochs):
        start_time = time.time()

        if args.graph_model:
            train_loss, train_acc, _, _, train_fscore, train_fscore_micro, _, _, _, _ = train_or_eval_graph_model(model, loss_function,
                                                                                                 loss_function_edge,
                                                                                                 train_loader,
                                                                                                 e, cuda,
                                                                                                 optimizer, train=True)
            valid_loss, valid_acc, _, _, valid_fscore, valid_fscore_micro, _, _, _, _ = train_or_eval_graph_model(model, loss_function,
                                                                                                 loss_function_edge,
                                                                                                 valid_loader,
                                                                                                 e, cuda)
            test_loss, test_acc, test_label, test_pred, test_fscore, test_fscore_micro, _, _, _, _ = train_or_eval_graph_model(model,
                                                                                                               loss_function,
                                                                                                               loss_function_edge,
                                                                                                               test_loader,
                                                                                                               e, cuda)
            all_fscore.append(test_fscore)
            all_fscore_micro.append(test_fscore_micro)
        ...
        logger.info(
            'epoch: {}, train_loss: {}, train_acc: {}, train_fscore: {}, valid_loss: {}, valid_acc: {}, valid_fscore: {}, test_loss: {}, test_acc: {}, test_fscore: {}, test_fscore_micro: {}, time: {} sec'. \
            format(e + 1, train_loss, train_acc, train_fscore, valid_loss, valid_acc, valid_fscore, test_loss, test_acc,
                   test_fscore, test_fscore_micro, round(time.time() - start_time, 2)))
    ...
    logger.info('Epoch {} : Micro-F-Score: {}'.format(all_fscore_micro.index(max(all_fscore_micro)) + 1, max(all_fscore_micro)))
