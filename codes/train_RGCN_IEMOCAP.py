...
def train_or_eval_graph_model(model, loss_function, loss_function_edge, dataloader, epoch, cuda, optimizer=None, train=False):
    losses, preds, labels = [], [], []
    scores, vids = [], []
...
if __name__ == '__main__':
    ...
    path = '../experiments/ResidualGCN_baseLSTM_IEMOCAP/'
    ...
    if args.tensorboard:
        ...
        writer = SummaryWriter('../tb_logger/ResidualGCN_baseLSTM_IEMOCAP')
    ...
    for e in range(n_epochs):
        ...
        if args.graph_model:
            train_loss, train_acc, _, _, train_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, loss_function_edge, train_loader, e, cuda, optimizer, True)
            valid_loss, valid_acc, _, _, valid_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, loss_function_edge, valid_loader, e, cuda)
            test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, loss_function_edge, test_loader, e, cuda)
    ...
    print ('Epoch {} : F-Score: {}'.format(all_fscore.index(max(all_fscore)), max(all_fscore)))
